    ...
    <main className="flex-1 p-4 lg:p-8 w-full overflow-x-hidden space-y-8 md:space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 bg-slate-900 text-white p-6 md:p-12 rounded-3xl md:rounded-[3rem] shadow-2xl relative overflow-hidden">
         ...
         <div className="relative z-10 bg-white/10 backdrop-blur-2xl border border-white/20 p-8 rounded-3xl flex items-center gap-6 shadow-2xl">
            <div className="h-16 w-16 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground shadow-lg shadow-primary/40 animate-bounce">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-12 h-14 rounded-2xl text-lg border-2 focus:border-primary shadow-sm"
          value={searchQuery}
      ...
      <Tabs defaultValue="books" className="space-y-10">
         <div className="flex justify-center">
            <TabsList className="bg-muted p-1 rounded-2xl h-14 shadow-inner">
               <TabsTrigger value="books" className="rounded-xl px-10 font-black uppercase tracking-widest text-xs h-full">Library Items</TabsTrigger>
               <TabsTrigger value="courses" className="rounded-xl px-10 font-black uppercase tracking-widest text-xs h-full">Premium Courses</TabsTrigger>
               <TabsTrigger value="perks" className="rounded-xl px-10 font-black uppercase tracking-widest text-xs h-full">Student Perks</TabsTrigger>
            </TabsList>
         </div>

         <TabsContent value="books" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               {resources.map((res) => {
                 const isOwned = myResources.includes(res.id);
                 return (
                   <Card key={res.id} className={cn(
                     "group rounded-[2.5rem] border-2 transition-all hover:scale-[1.02] hover:shadow-2xl overflow-hidden",
                     isOwned && "border-primary/20 opacity-80"
                      ...
                      <CardHeader className="p-8">
                         <CardTitle className="text-2xl font-black uppercase tracking-tighter group-hover:text-primary transition-colors">{res.title}</CardTitle>
                         <CardDescription className="text-sm font-medium leading-relaxed">{res.description}</CardDescription>
                      </CardHeader>
                      <CardFooter className="p-8 pt-0 border-t bg-slate-50/50 flex justify-between items-center h-20">
                         <div className="flex items-center gap-2 font-black text-xl">
                            <Zap className="h-5 w-5 fill-primary text-primary" /> {res.price_points}
                         </div>
                         {isOwned ? (
                           <Badge className="bg-emerald-600 text-white font-black h-10 px-6 rounded-full gap-2">
                              <CheckCircle2 className="h-4 w-4" /> OWNED
                           </Badge>
                         ) : (
                           <Button className="rounded-xl font-black uppercase tracking-widest px-8 h-12 shadow-xl shadow-primary/20" onClick={() => handlePurchase(res.id, 'resource', res.price_points, res.title)}>
                              Buy Item
                           </Button>
                         )}
                      </CardFooter>
         ...
         <TabsContent value="courses" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
               ...
               {courses.filter(c => c.slug !== 'web-dev').map((course) => {
                 ...
                 return (
                   <Card key={course.id} className={cn(
                     "group rounded-[2.5rem] border-2 transition-all hover:shadow-2xl flex flex-col overflow-hidden",
                     isOwned && "border-primary/20"
                   )}>
                      <div className="h-40 bg-gradient-to-r from-primary/10 to-transparent p-10 flex items-center justify-between">
                         <div className="h-20 w-20 rounded-3xl bg-white shadow-xl flex items-center justify-center text-primary">
                            <Sparkles className="h-10 w-10" />
                         </div>
                         <div className="text-right">
                            <Badge variant="outline" className="font-black uppercase text-primary border-primary">PREMIUM ACCESS</Badge>
                         </div>
                      </div>
                      <CardHeader className="p-10 pt-6">
                         <CardTitle className="text-3xl font-black uppercase tracking-tighter leading-none">{course.name}</CardTitle>
                         <CardDescription className="text-lg font-medium mt-2">{course.description}</CardDescription>
                      </CardHeader>
                      <CardFooter className="p-10 pt-0 mt-auto flex justify-between items-center gap-6">
                         <div className="space-y-1">
                            <div className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Unlocking Fee</div>
                            <div className="text-3xl font-black flex items-center gap-2">
                               <Zap className="h-6 w-6 fill-primary text-primary" /> {price}
                            </div>
                         </div>
                         {isOwned ? (
                            <Button variant="outline" className="h-14 px-10 rounded-2xl font-black uppercase border-emerald-500 text-emerald-600 gap-2" disabled>
                               <CheckCircle2 className="h-5 w-5" /> Course Unlocked
                            </Button>
                         ) : (
                           <Button className="h-14 px-10 rounded-2xl font-black uppercase tracking-widest shadow-2xl shadow-primary/30" onClick={() => handlePurchase(course.id, 'course', price, course.name)}>
                              Unlock Course <ArrowRight className="ml-2 h-5 w-5" />
                           </Button>
                         )}
                      </CardFooter>
                   </Card>
         ...
         <TabsContent value="perks" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               {SHOP_ITEMS.map((perk) => (
                 <Card key={perk.id} className="rounded-[2rem] border-2 group hover:border-amber-500/50 transition-all flex flex-col items-center text-center p-8">
                    <div className="h-24 w-24 rounded-[2rem] bg-amber-500/10 flex items-center justify-center text-4xl mb-6 shadow-inner group-hover:scale-110 transition-transform">
                       {perk.icon}
                    </div>
                    <h3 className="text-xl font-black uppercase tracking-tighter mb-2">{perk.name}</h3>
                    <p className="text-xs text-muted-foreground font-medium mb-8 leading-relaxed px-4">{perk.description}</p>
